...
def run():
    ...
    print(1)
    print(os.getcwd())
    loaded_model = pickle.load(open('modules\\RF_30Trees_50depth_balanced.sav', 'rb'))  # model path
    print(2)
    labelMapping = {0: 'text',
                    1: 'titlu',
                    2: 'editura',
    ...
    with open('outputPredictions.csv', 'w') as myfile:
        ...
        for index, row in readData.iterrows():
            modelFeatures = ['pageId', 'x', 'y', 'page_h', 'page_w', 'page_cov', 'hasISBN', 'hasEditura', 'normRows',
                             'normWords']
            rawText = row['rawText']
            print('\n@@@'+str(row[modelFeatures])+'@@@\n')
